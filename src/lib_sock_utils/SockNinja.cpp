...
SockNinja::SockNinja(string ninja_type, int port)
{
  ...
  m_warning_bad_nmea_len  = true;
  m_warning_bad_nmea_nend = true;
  m_warning_bad_nmea_rend = true;
  m_warning_bad_nmea_form = true;
  m_warning_bad_nmea_chks = true;
  m_warning_bad_nmea_key  = true;
...
bool SockNinja::isValidNMEA(string str, bool strict)
{
  ...
  if(len < 12) {
    if(m_warning_bad_nmea_len) {
      addWarning("NMEA bad len: [" + str + "]");
    }
    return(false);
  }
  if(str[len-1] != '\n')  {
    if(m_warning_bad_nmea_nend) {
      int c = (int)(str[len-1]);
      addWarning("NMEA bad nend: " + intToString(c));
    }
    return(false);
  }
  if(str[len-2] != '\r') {
    if(m_warning_bad_nmea_rend) {
      int c = (int)(str[len-2]);
  ...
  if((str[0] != '$') || (str[6] != ',') || (str[len-5] != '*')) {
    if(m_warning_bad_nmea_form) {
      addWarning("NMEA bad $,*");
    }
    return(false);
  }
  if(!isCheckSumChar(str[len-4]) || !isCheckSumChar(str[len-3])) {
    if(m_warning_bad_nmea_chks) {    
      addWarning("NMEA HH");
    }
    return(false);
  }
...
bool SockNinja::setupListening()
{
  ...
  // Mod mikerb 3/30/25 in moving code to git
